...
exports.default = function (client, stz, query) {
    _model2.default.auth.findOne({ username: client.jid.user }, function (err, user) {
        var profile = _ltx2.default.parse("<iq to='" + stz.attrs.from + "' from='" + stz.attrs.to + "' id='" + stz.attrs.id + "' type='result'>" + "<query xmlns='urn:cryonline:k01'>" + "<invitation_result result='17' user='rodrigoza1' is_follow='0' user_id='26430'/>" + "</query>" + "</iq>");
        client.send(profile);
    });
};
