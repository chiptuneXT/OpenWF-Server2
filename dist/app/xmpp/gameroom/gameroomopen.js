...
function getMission(key, cb) {
    _model2.default.missions.findOne({ mission_key: key }, function (err, mission) {
...
exports.default = function (client, stz, query) {
    _model2.default.profile.findOne({ username: client.jid.user }, function (err, user) {
        ...
        var GameRoom = new _model2.default.gameroomopen({
            ...
            core: (_core = {
                ...
                region_id: 'global'
            }]), _defineProperty(_core, 'team_colors', [{ id: '1', color: '4294907157' }, { id: '2', color: '4279655162' }]), _core),
            session: {
                id: '',
                status: query.status,
                game_progress: '0',
        ...
        GameRoom.save(function (err) {
            ...
            getMission(GameRoom.mission.mission_key, function (mission_pl) {
                ...
                var gameroomopen = _ltx2.default.parse("<iq to='" + stz.attrs.from + "' from='" + stz.attrs.to + "' id='" + stz.attrs.id + "' type='result'>" + "<query xmlns='urn:cryonline:k01'>" + "<gameroom_open>" + "<game_room room_id='" + GameRoom.room_id + "' room_type='" + GameRoom.room_type + "'>" + "<core teams_switched='" + GameRoom.core.teams_switched + "' room_name='" + GameRoom.core.room_name + "' private='" + GameRoom.core.private + "' players='" + GameRoom.core.players + "' can_start='" + GameRoom.core.can_start + "' team_balanced='" + GameRoom.core.team_balanced + "' min_ready_players='" + GameRoom.core.min_ready_players + "' revision='" + GameRoom.core.revision + "'>" + "<players>" + getPlayers(GameRoom.core.players) + "</players>" + "<playersReserved/>" + "<team_colors>" + getTeamColors(GameRoom.core.team_colors) + "</team_colors>" + "</core>" + "<session id='" + GameRoom.session.id + "' status='" + GameRoom.session.status + "' game_progress='" + GameRoom.session.game_progress + "' start_time='" + GameRoom.session.start_time + "' revision='" + GameRoom.session.revision + "'/>" + "<custom_params friendly_fire='" + GameRoom.custom_params.friendly_fire + "' enemy_outlines='" + GameRoom.custom_params.enemy_outlines + "' auto_team_balance='" + GameRoom.custom_params.auto_team_balance + "' dead_can_chat='" + GameRoom.custom_params.dead_can_chat + "' join_in_the_process='" + GameRoom.custom_params.join_in_the_process + "' max_players='" + GameRoom.custom_params.max_players + "' round_limit='" + GameRoom.custom_params.round_limit + "' class_restriction='" + GameRoom.custom_params.class_restriction + "' inventory_slot='" + GameRoom.custom_params.inventory_slot + "' revision='" + GameRoom.custom_params.revision + "'/>" + mission_pl + "<kick_vote_params success='" + GameRoom.kick_vote_params.success + "' timeout='" + GameRoom.kick_vote_params.timeout + "' cooldown='" + GameRoom.kick_vote_params.cooldown + "' revision='" + GameRoom.kick_vote_params.revision + "'/>" + "<room_master master='" + GameRoom.room_master.master + "' revision='" + GameRoom.room_master.revision + "'/>" + "</game_room>" + "</gameroom_open>" + "</query>" + "</iq>");
...
//# sourceMappingURL=gameroomopen.js.map